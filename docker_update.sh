...
docker-compose -f ./prod/compose.yaml up -d --no-deps --scale $SERVICE_NAME=2 --no-recreate $SERVICE_NAME
...
NGINX_CONTAINER_ID=$(docker ps --format "table {{.ID}}  {{.Names}}  {{.CreatedAt}}" | grep nginx | tail -n 1 | awk -F  "  " '{print $1}')
docker exec -t $NGINX_CONTAINER_ID nginx -t
docker exec -t $NGINX_CONTAINER_ID nginx -s reload
...
docker-compose -f ./prod/compose.yaml up -d --no-deps --scale $SERVICE_NAME=1 --no-recreate $SERVICE_NAME